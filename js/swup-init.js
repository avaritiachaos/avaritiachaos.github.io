...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var swup = new Swup({
      ...
      // 游戏「试玩页」的 importmap + ES module 入口注入在 <head>（define "head"）里，
      // 而 Swup 只替换 #swup、从不重跑 <head>，PJAX 进入时引擎永远不会启动
      // （且 importmap 也不存在，import 'three' 会失败）。试玩页必须整页加载。
      // 试玩页 = /games/<slug>/（有具体游戏 slug）；游戏库 /games/ 本身是普通页，照常 PJAX。
      ignoreVisit: function (url, opts) {
        var el = opts && opts.el;
        if (el && el.closest && el.closest('[data-no-swup]')) return true; // 保留默认行为
        return /\/games\/[^/?#]+/.test(url);   // 匹配 /games/chromatic-drift/ 这类试玩页
      }
    });
    ...
    function reinitThemeComponents() {
      ...
      // --- 3d-pre3. Archive Showcase：重新初始化时间线动画 ---
      if (window.__archiveShowcase && window.__archiveShowcase.init) {
        window.__archiveShowcase.init();
      }

      // --- 3d-pre4. Mobile Reading Chrome：重新评估滚动方向状态（不重复绑定 scroll） ---
      if (window.__mobileChrome && window.__mobileChrome.init) {
        window.__mobileChrome.init();
      }
    ...
    swup.hooks.on('page:view', function () {
      ...
      // 更新归档沉浸模式状态
      if (typeof window.updateArchiveImmersiveState === 'function') {
        window.updateArchiveImmersiveState();
      }

      // === 游戏生命周期：离开 game 页时释放 WebGL 资源 ===
      // 若新页面不再是游戏页，且引擎仍在运行，则强制 dispose，防止显存泄漏 / 后台 RAF。
      var isGamePage = !!document.getElementById('game-root');
      if (!isGamePage && window.__gameEngine && typeof window.__gameEngine.dispose === 'function') {
        try {
          window.__gameEngine.dispose();
        } catch (e) { console.warn('[PJAX] game dispose error:', e); }
        window.__gameEngine = null;
      }
    });
